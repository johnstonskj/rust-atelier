...
use atelier_core::model::shapes::{Member, Shape, ShapeInner, Trait, Valued};
use atelier_core::model::values::{Key, NodeValue, Number};
use atelier_core::model::{Annotated, Model, Named, ShapeID};
use serde_json::{to_writer_pretty, Map, Number as JsonNumber, Value};
use std::io::Write;
...
impl<'a> ModelWriter<'a> for JsonWriter {
    fn write(&mut self, w: &mut impl Write, model: &'a Model) -> Result<()> {
        ...
        top.insert(K_SHAPES.to_string(), self.shapes(model));
    ...
    fn shape(&self, shape: &'a Shape) -> Value {
        ...
        match shape.inner() {
            ShapeInner::SimpleType(v) => {
                let _ = shape_map.insert(K_TRAITS.to_string(), Value::String(v.to_string()));
            }
            ShapeInner::List(v) => {
                let _ = shape_map.insert(K_TRAITS.to_string(), Value::String(V_LIST.to_string()));
            ...
            ShapeInner::Map(v) => {
                ...
                let _ = shape_map.insert("key".to_string(), self.reference(v.key()));
                let _ = shape_map.insert("value".to_string(), self.reference(v.value()));
            }
